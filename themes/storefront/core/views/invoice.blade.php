            ...
            @foreach ($invoice['items'] as $item)
                <li class="flex items-start justify-between gap-4 px-5 py-3 text-sm">
                    <span>
                        ...
                        @endif
                        @if ($item['detail'])
                            <span class="mt-0.5 block text-xs text-content-muted whitespace-pre-line">{{ $item['detail'] }}</span>
                        @endif
                    </span>
                        ...
                        <label class="flex cursor-pointer items-start gap-3 rounded-[var(--radius-md)] border border-line px-4 py-3 text-sm has-checked:border-brand">
                            ...
                            <span>
                                ...
                                @if ($gateway['instructions'])
                                    <span class="mt-1 block text-xs leading-relaxed text-content-muted whitespace-pre-line">{{ $gateway['instructions'] }}</span>
                                @endif
                            </span>
                        </label>
                    @endforeach
