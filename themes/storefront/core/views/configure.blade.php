...
@section('content')
    ...
    <h1 class="mt-4 text-page leading-[1.1] font-semibold tracking-tighter text-balance sm:text-display">
    ...
    <form method="POST" action="{{ route('storefront.cart.store') }}" class="mt-8 grid gap-10 lg:grid-cols-12 lg:gap-14">
        ...
        <div class="flex flex-col gap-8 lg:col-span-7">
            <fieldset>
                <legend class="text-body font-semibold">{{ __('ordering.configure.cycle') }}</legend>
                ...
                <div class="mt-3 grid gap-2 sm:grid-cols-2">
                    @foreach ($product['cycles'] as $index => $cycle)
                        <label class="flex cursor-pointer items-center justify-between gap-3 rounded-[var(--radius-md)] border border-line bg-surface-primary px-4 py-3 text-body has-checked:border-brand">
            ...
            @if ($product['requiresDomain'])
                <div class="flex flex-col gap-2">
                    <label for="domain" class="text-body font-semibold">{{ __('ordering.configure.domain') }}</label>
                    <input
                        id="domain"
                        name="domain"
                        type="text"
                    ...
                    >
                    <p class="text-chrome text-content-muted">{{ __('ordering.configure.domain_hint') }}</p>
                </div>
            @endif

            @foreach ($product['optionGroups'] as $group)
                <fieldset>
                    <legend class="text-body font-semibold">
                        {{ $group['name'] }}
                        @unless ($group['isRequired'])
                            <span class="ml-1.5 text-chrome font-normal text-content-subtle">optional</span>
                        @endunless
                    </legend>

                    @if ($group['description'])
                        <p class="mt-1 text-body text-content-muted">{{ $group['description'] }}</p>
                    @endif
                    ...
                    @if ($group['type'] === 'quantity')
                        <input
                            ...
                            class="mt-3 w-28 rounded-[var(--radius-sm)] border border-line bg-surface-primary px-3 py-2 text-body tabular-nums"
                        >
                    @else
                        <div class="mt-3 divide-y divide-line rounded-[var(--radius-md)] border border-line">
                            @foreach ($group['options'] as $option)
                                <label class="flex cursor-pointer items-center justify-between gap-4 px-4 py-2.5 text-body">
                                    <span class="flex items-center gap-3">
                                        <input
                                            type="radio"
            ...
            @if (! empty($product['addons']))
                <fieldset>
                    <legend class="text-body font-semibold">{{ __('ordering.configure.addons') }}</legend>

                    <div class="mt-3 divide-y divide-line rounded-[var(--radius-md)] border border-line">
                        @foreach ($product['addons'] as $addon)
                            <label class="flex cursor-pointer items-start justify-between gap-4 px-4 py-3 text-body">
        ...
        <div class="lg:col-span-5">
            <div class="rounded-[var(--radius-xl)] border border-line bg-surface-primary p-6 lg:sticky lg:top-8">
                <h2 class="text-body font-semibold">{{ $product['name'] }}</h2>

                @if ($product['tagline'])
                    <p class="mt-1 text-body leading-relaxed text-content-muted">{{ $product['tagline'] }}</p>
                @endif

                <div class="mt-5 flex flex-col gap-2">
                    <label for="quantity" class="text-chrome font-medium text-content-muted">
                    ...
                    <input
                        ...
                        min="1"
                        max="100"
                        class="w-24 rounded-[var(--radius-sm)] border border-line bg-background px-3 py-2 text-body tabular-nums"
                    >
                </div>
