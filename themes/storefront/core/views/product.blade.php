...
@section('content')
    ...
    <div class="mt-4 grid gap-12 lg:grid-cols-12 lg:gap-16">
        <div class="lg:col-span-7">
            <h1 class="text-3xl leading-[1.1] font-semibold tracking-tighter text-balance sm:text-4xl">
                {{ $product['name'] }}
            </h1>

            @if ($product['tagline'])
                <p class="mt-4 max-w-[56ch] text-base leading-relaxed text-content-muted">
                    {{ $product['tagline'] }}
                </p>
            @endif

            @if ($product['description'])
                <div class="mt-6 max-w-[64ch] text-sm leading-relaxed whitespace-pre-line">
                    {{ $product['description'] }}
                </div>
            @endif
            ...
            @if (! empty($product['features']))
                <section class="mt-10">
                    <h2 class="text-sm font-semibold">{{ __('catalog.storefront.features') }}</h2>
                    <ul class="mt-3 grid gap-2 text-sm text-content-muted sm:grid-cols-2">
            ...
            @foreach ($product['optionGroups'] as $group)
                <section class="mt-10">
                    <h2 class="text-sm font-semibold">
                        {{ $group['name'] }}
                        @if ($group['isRequired'])
                            <span class="ml-1.5 text-xs font-normal text-content-subtle">
                    ...
                    @if ($group['description'])
                        <p class="mt-1 text-sm leading-relaxed text-content-muted">{{ $group['description'] }}</p>
                    @endif
                        ...
                        @foreach ($group['options'] as $option)
                            <li class="flex items-center justify-between gap-4 px-4 py-2.5 text-sm">
                                <span>
                                    {{ $option['label'] }}
                                    @if ($option['isDefault'])
                                        <span class="ml-1.5 text-xs text-content-subtle">
            ...
            @if (! empty($product['addons']))
                <section class="mt-10">
                    <h2 class="text-sm font-semibold">{{ __('catalog.addons.title') }}</h2>
                    <ul class="mt-3 divide-y divide-line rounded-[var(--radius-md)] border border-line">
        ...
        <div class="lg:col-span-5">
            <div class="rounded-[var(--radius-lg)] border border-line bg-surface-raised p-6 shadow-(--shadow-panel) lg:sticky lg:top-8">
                ...
                <ul class="mt-4 divide-y divide-line">
                    @foreach ($product['cycles'] as $cycle)
                        <li class="flex items-baseline justify-between gap-4 py-3">
                            <span class="text-sm text-content-muted">{{ $cycle['cycleLabel'] }}</span>
                            <span class="text-right">
                                <span class="text-base font-semibold tabular-nums">{{ $cycle['amount'] }}</span>
                                @if ($cycle['setup'])
                ...
                <div class="mt-6">
                    @if ($product['soldOut'])
                        <span class="inline-flex w-full items-center justify-center rounded-[var(--radius-sm)] border border-line px-5 py-2.5 text-sm font-semibold text-content-subtle">
