...
@section('content')
    <div class="max-w-[52ch]">
        ...
        @if ($invoice)
            <div class="mt-6 flex flex-wrap items-baseline justify-between gap-2 rounded-[var(--radius-sm)] border border-line bg-surface-secondary px-3 py-2 text-body">
                <span>{{ __('billing.invoices.raised', ['number' => $invoice['number']]) }}</span>
                <span class="tabular-nums font-semibold">{{ $invoice['due'] }}</span>
            </div>
        @endif

        <p class="mt-6 text-body leading-relaxed text-content-muted">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
            ...
            <a
                href="{{ url('/client') }}"
                class="pressable inline-flex items-center rounded-[var(--radius-sm)] {{ $invoice ? 'border border-line-strong' : 'bg-brand text-content-inverse' }} px-5 py-2.5 text-body font-semibold"
            >
