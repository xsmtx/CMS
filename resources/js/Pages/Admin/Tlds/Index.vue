    ...
    <AppCard>
      ...
      >
        <tr v-for="tld in tlds" :key="tld.id">
          <td class="px-4 py-3 font-medium">.{{ tld.extension }}</td>
          <td class="text-content-muted px-4 py-3">{{ tld.registrar ?? '—' }}</td>
          <td class="px-4 py-3">{{ tld.minYears }}–{{ tld.maxYears }} years</td>
          <td class="text-content-muted px-4 py-3">{{ priceSummary(tld) }}</td>
          <td class="px-4 py-3 tabular-nums">{{ tld.domains }}</td>
