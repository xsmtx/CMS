...
/**
 * Deleting a rule used to happen on the first click, from a button labelled
 * with a multiplication sign.
 *
 * A tax rule decides what a customer is charged. Removing one silently is the
 * one change on this screen nobody would notice until an invoice came out
 * wrong, so it asks — and the reason goes to the audit record, because the
 * next person reading it will be trying to work out why VAT stopped.
 */
function remove(reason: string | null): void {
  ...
  router.delete(`/admin/tax/rules/${rule.id}`, {
    data: { reason },
    preserveScroll: true,
    onFinish: () => {
      removing.value = null
...
<template>
  ...
  <AdminLayout :heading="t('tax.title')">
    <template #header>
      <PageHeader :title="t('tax.title')" :description="t('tax.intro')" />
    </template>
    ...
    <div class="flex flex-col gap-8">
      ...
      <DetailSection
        ...
        :description="t('tax.rules.intro')"
        :divided="rules.length === 0"
      >
        <EmptyState
          v-if="rules.length === 0"
          :title="t('tax.rules.title')"
        ...
        <AppTable v-else name="tax-rules" :columns="columns">
          <AppTableRow v-for="rule in rules" :key="rule.id">
            <td data-col="name" class="font-medium">{{ rule.name }}</td>
            <td data-col="rate" class="numeric tabular-nums">{{ rule.rate }}%</td>
            <td data-col="place">{{ place(rule) }}</td>
            <td data-col="level">
              <span class="flex flex-wrap items-center gap-x-2">
                <span>{{ rule.level }}</span>
                <!-- The word, not a plus sign. A compound level is charged on
                     the amount *plus* the level below it, which is not a thing
                     a symbol can say. -->
                <AppBadge v-if="rule.compound" tone="info">
                  {{ t('tax.rules.compound_badge') }}
                </AppBadge>
              </span>
            </td>
            <td data-col="applies" class="text-content-muted text-chrome">
              {{ labelOf(options.appliesTo, rule.appliesTo) }}
            </td>
            <td data-col="customer" class="text-content-muted text-chrome">
              {{ labelOf(options.customerKinds, rule.customerKind) }}
            </td>
            <td data-col="dates" class="text-content-muted text-chrome tabular-nums">
              {{ dates(rule) }}
            </td>
            <td data-col="state">
              <!-- Whether a rule fires is a state, so it takes a mark and a
                   word rather than a coloured chip. -->
              <AppStatus
                :tone="rule.isActive ? 'healthy' : 'neutral'"
                :label="rule.isActive ? t('tax.rules.active') : t('tax.rules.inactive')"
              />
            </td>
            <td data-col="actions" class="text-right">
              <span class="row-actions inline-flex items-center gap-1">
                ...
                <AppButton size="sm" variant="danger-subtle" @click="removing = rule">
                  {{ t('tax.rules.delete') }}
                </AppButton>
              </span>
            </td>
      ...
      <DetailSection
        :title="editing === null ? t('tax.rules.add') : t('tax.rules.edit')"
        :description="t('tax.rules.form_intro')"
      >
        <form class="flex flex-col gap-4" @submit.prevent="save">
          ...
          <div class="flex items-center gap-2">
            <AppButton type="submit" variant="primary" :loading="form.processing">
              {{ editing === null ? t('tax.rules.add') : t('tax.rules.save') }}
            </AppButton>
            <AppButton v-if="editing !== null" variant="ghost" @click="reset">
              {{ t('ui.confirm.cancel') }}
            </AppButton>
          </div>
        </form>
      ...
      <DetailSection :title="t('tax.settings.title')" :description="t('tax.settings.intro')">
      ...
      <DetailSection :title="t('tax.preview.title')" :description="t('tax.preview.intro')">
        <form class="flex flex-col gap-4" @submit.prevent="tryIt">
          ...
          <div v-if="preview" class="border-line flex flex-col gap-2 rounded-lg border p-4">
    ...
    <AppConfirm
      :open="removing !== null"
      level="high-risk"
      :title="t('tax.rules.delete_title')"
      :description="t('tax.rules.delete_detail')"
      :confirm-label="t('tax.rules.delete_confirm')"
