  ...
  >
    ...
    >
      <tr v-for="item in requests.data" :key="item.id">
        ...
        <td class="text-content-muted px-4 py-3">{{ item.token ?? '—' }}</td>
        <td class="px-4 py-3">
        ...
        <td class="text-content-muted px-4 py-3 tabular-nums">{{ item.durationMs }}ms</td>
        <td class="text-content-muted px-4 py-3 font-mono text-xs">{{ item.ip ?? '—' }}</td>
